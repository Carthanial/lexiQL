...
const resolvers = {
  ...
  Person: {
    ...
    species: (people) => {
      const query =
        'SELECT species.* FROM species LEFT OUTER JOIN people ON species._id = people._id WHERE people._id = $1';
      const values = [people._id];
      return db
        .query(query, values)
        .then((data) => data.rows)
    ...
    planets: (people) => {
      const query =
        'SELECT planets.* FROM planets LEFT OUTER JOIN people ON planets._id = people._id WHERE people._id = $1';
      const values = [people._id];
      return db
        .query(query, values)
        .then((data) => data.rows)
  ...
  Species: {
    ...
    planets: (species) => {
      const query =
        'SELECT planets.* FROM planets LEFT OUTER JOIN species ON planets._id = species._id WHERE species._id = $1';
      const values = [species._id];
      return db
        .query(query, values)
        .then((data) => data.rows)
